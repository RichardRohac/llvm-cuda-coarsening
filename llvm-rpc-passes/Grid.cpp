...
std::string GridAnalysis::THREAD_INDEX = "threadIdx";
std::string GridAnalysis::BLOCK_INDEX = "blockIdx";
std::string GridAnalysis::BLOCK_DIM = "blockDim";
std::string GridAnalysis::GRID_DIM = "gridDim";
int GridAnalysis::DIRECTION_NUMBER = 3;

GridAnalysis::GridAnalysis() : FunctionPass(ID) {}

void GridAnalysis::getAnalysisUsage(AnalysisUsage &au) const {
  au.setPreservesAll();
}

bool GridAnalysis::runOnFunction(Function &function) {
  Function *functionPtr = (Function *)&function;
  init();
  findKernelIndexByNameAllDirs(THREAD_INDEX, functionPtr);
  findKernelIndexByNameAllDirs(BLOCK_INDEX, functionPtr);
  findKernelIndexByNameAllDirs(BLOCK_DIM, functionPtr);
  findKernelIndexByNameAllDirs(GRID_DIM, functionPtr);

  return false;
}

// -----------------------------------------------------------------------------
InstVector GridAnalysis::getTids() {
  InstVector result;
...
void NDRange::findKernelIndexByNameAllDirs(std::string calleeName,
                                           Function *caller) {
  for (int direction = 0; direction < DIRECTION_NUMBER; ++direction) {
    std::map<std::string, InstVector> &dirInsts = oclInsts[direction];
...
char NDRange::ID = 0;
static RegisterPass<GridAnalysis> X("cuda-grid-analysis", "CUDA Grid Analysis");
 */